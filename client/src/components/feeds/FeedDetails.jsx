export default function AnnouncementDetails({ data, setFeedDetailPopUp, setEditFeed, handleDeleteFeed }) {
   ...
   return (
      <div className="w-11/12 sm:w-5/6 md:w-2/3 lg:w-1/3 bg-bgSecondary rounded-3xl p-5 z-20">
         <div className="flex flex-col">
            ...
            {!data?.tags[0]?.replace(" ", "") ? <p className="text-gray-400 text-sm p-[4px] rounded-md bg-slate-800">No tags</p> : data?.tags[0]?.split(" ").map((tag, index)=>(
               <p key={tag} className={index === 0 && 'text-red-400' || index%2 === 0 && 'text-green-500' || (index%3 === 0 ? 'text-pink-500' : 'text-blue-400')}>#{tag}</p>
            ))}
            </div>
