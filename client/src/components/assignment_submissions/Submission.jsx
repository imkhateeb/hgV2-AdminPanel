...
export default function Submission({ submission, assignmentId }) {
  ...
  return (
    <>
      <div
        className="flex z-30 border-t-2 py-4 w-full hover:bg-slate-800 hover:bg-opacity-50 transition-all duration-200 ease-in"
      >
        ...
        <div className="w-[20%] flex relative gap-1 items-center text-[16px] font-semibold">
          {submission?.submittedAt.split('T')[0] + " : " + submission?.submittedAt.split('T')[1].split('.')[0]}
        </div>
        <div className="w-[15%] flex relative gap-1 items-center text-[16px] font-semibold">
          {submission?.submitted?.verified
            ? <RiVerifiedBadgeFill
            fontSize={24}
             className="text-green-500 text-xl font-bold" /> : <BsPatchQuestionFill
            fontSize={24}
             className="text-red-500 text-xl font-bold" />}
        </div>
        <div className="flex w-[15%] gap-2 flex justify-center">
          {!submission?.submitted?.verified && (
